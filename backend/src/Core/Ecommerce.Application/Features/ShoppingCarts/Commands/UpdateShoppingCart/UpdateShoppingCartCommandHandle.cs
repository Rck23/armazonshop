...
public class UpdateShoppingCartCommandHandle : IRequestHandler<UpdateShoppingCartCommand, ShoppingCartVm>
{

    private readonly IUnitOfWork _unitOfWork;
    private readonly IMapper _mapper;

    public UpdateShoppingCartCommandHandle(IUnitOfWork unitOfWork, IMapper mapper)
    {
    ...
    public async Task<ShoppingCartVm> Handle(UpdateShoppingCartCommand request, CancellationToken cancellationToken)
    {
        ...
        if(resultado <= 0)
        {
            throw new Exception("No se pudo agregar ningun producto al carrito de compras");
        }
